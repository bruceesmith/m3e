...
func GenerateTests(directory string, modName string, module parser.Module, version string, date string) (err error) {
	gleam := GleamModule{}
	logger.TraceID("tests", fmt.Sprintf("Module %s: %s\n", modName, module.Description))
	...
	gleam.slots, err = slots(module.Slots, modName)
	if err != nil {
		return fmt.Errorf("processing of %s failed: %w", modName, err)
	}
