//// PanGesture unit tests
////
//// This file was generated by m3e/generator
////
////          DO NOT EDIT
////

import gleam/list
import gleam/option.{None, Some}
import gleeunit/should
import lustre/attribute
import lustre/element
import m3e/gesture_input_button
import m3e/pan_gesture.{Config}
import m3e/pan_gesture_activation_mode
import m3e/pan_gesture_lock_axis
import m3e/pointer_type

pub fn pan_gesture_default_config_test() {
  let cases = [
    Config(
      for: None,
      buttons: [gesture_input_button.Primary],
      pointer_types: [pointer_type.Mouse, pointer_type.Pen, pointer_type.Touch],
      disabled: pan_gesture.IsNotDisabled,
      priority: "1",
      pointers: 1.0,
      activation_mode: pan_gesture_activation_mode.Press,
      min_displacement: 4.0,
      lock_axis: pan_gesture_lock_axis.None,
      axis_threshold: 8.0,
      delta_threshold: 0.0,
      max_press_interval: 120.0,
    ),
  ]

  list.each(cases, fn(c) {
    let expected = c

    pan_gesture.default_config()
    |> should.equal(expected)
  })
}

pub fn pan_gesture_from_config_test() {
  let cases = [
    #(
      pan_gesture.Config(
        for: Some("test"),
        buttons: [gesture_input_button.Secondary],
        pointer_types: [pointer_type.Mouse],
        disabled: pan_gesture.IsDisabled,
        priority: "test",
        pointers: 42.0,
        activation_mode: pan_gesture_activation_mode.Move,
        min_displacement: 42.0,
        lock_axis: pan_gesture_lock_axis.X,
        axis_threshold: 42.0,
        delta_threshold: 42.0,
        max_press_interval: 42.0,
      ),
      pan_gesture.new()
        |> pan_gesture.for(Some("test"))
        |> pan_gesture.buttons([gesture_input_button.Secondary])
        |> pan_gesture.pointer_types([pointer_type.Mouse])
        |> pan_gesture.disabled(pan_gesture.IsDisabled)
        |> pan_gesture.priority("test")
        |> pan_gesture.pointers(42.0)
        |> pan_gesture.activation_mode(pan_gesture_activation_mode.Move)
        |> pan_gesture.min_displacement(42.0)
        |> pan_gesture.lock_axis(pan_gesture_lock_axis.X)
        |> pan_gesture.axis_threshold(42.0)
        |> pan_gesture.delta_threshold(42.0)
        |> pan_gesture.max_press_interval(42.0),
    ),
  ]

  list.each(cases, fn(c) {
    let #(config, expected) = c

    pan_gesture.from_config(config)
    |> should.equal(expected)
  })
}

pub fn pan_gesture_new_test() {
  let cases = [
    pan_gesture.from_config(pan_gesture.Config(
      for: None,
      buttons: [gesture_input_button.Primary],
      pointer_types: [pointer_type.Mouse, pointer_type.Pen, pointer_type.Touch],
      disabled: pan_gesture.IsNotDisabled,
      priority: "1",
      pointers: 1.0,
      activation_mode: pan_gesture_activation_mode.Press,
      min_displacement: 4.0,
      lock_axis: pan_gesture_lock_axis.None,
      axis_threshold: 8.0,
      delta_threshold: 0.0,
      max_press_interval: 120.0,
    )),
  ]

  list.each(cases, fn(c) {
    let expected = c

    pan_gesture.new()
    |> should.equal(expected)
  })
}

pub fn pan_gesture_for_test() {
  let mod = pan_gesture.new()
  let cases = [
    #(
      Some("test"),
      pan_gesture.from_config(
        pan_gesture.Config(..pan_gesture.default_config(), for: Some("test")),
      ),
    ),
  ]

  list.each(cases, fn(c) {
    let #(field, expected) = c

    pan_gesture.for(mod, field)
    |> should.equal(expected)
  })
}

pub fn pan_gesture_buttons_test() {
  let mod = pan_gesture.new()
  let cases = [
    #(
      [gesture_input_button.Secondary],
      pan_gesture.from_config(
        pan_gesture.Config(..pan_gesture.default_config(), buttons: [
          gesture_input_button.Secondary,
        ]),
      ),
    ),
  ]

  list.each(cases, fn(c) {
    let #(field, expected) = c

    pan_gesture.buttons(mod, field)
    |> should.equal(expected)
  })
}

pub fn pan_gesture_pointer_types_test() {
  let mod = pan_gesture.new()
  let cases = [
    #(
      [pointer_type.Mouse],
      pan_gesture.from_config(
        pan_gesture.Config(..pan_gesture.default_config(), pointer_types: [
          pointer_type.Mouse,
        ]),
      ),
    ),
  ]

  list.each(cases, fn(c) {
    let #(field, expected) = c

    pan_gesture.pointer_types(mod, field)
    |> should.equal(expected)
  })
}

pub fn pan_gesture_disabled_test() {
  let mod = pan_gesture.new()
  let cases = [
    #(
      pan_gesture.IsDisabled,
      pan_gesture.from_config(
        pan_gesture.Config(
          ..pan_gesture.default_config(),
          disabled: pan_gesture.IsDisabled,
        ),
      ),
    ),
  ]

  list.each(cases, fn(c) {
    let #(field, expected) = c

    pan_gesture.disabled(mod, field)
    |> should.equal(expected)
  })
}

pub fn pan_gesture_priority_test() {
  let mod = pan_gesture.new()
  let cases = [
    #(
      "test",
      pan_gesture.from_config(
        pan_gesture.Config(..pan_gesture.default_config(), priority: "test"),
      ),
    ),
  ]

  list.each(cases, fn(c) {
    let #(field, expected) = c

    pan_gesture.priority(mod, field)
    |> should.equal(expected)
  })
}

pub fn pan_gesture_pointers_test() {
  let mod = pan_gesture.new()
  let cases = [
    #(
      42.0,
      pan_gesture.from_config(
        pan_gesture.Config(..pan_gesture.default_config(), pointers: 42.0),
      ),
    ),
  ]

  list.each(cases, fn(c) {
    let #(field, expected) = c

    pan_gesture.pointers(mod, field)
    |> should.equal(expected)
  })
}

pub fn pan_gesture_activation_mode_test() {
  let mod = pan_gesture.new()
  let cases = [
    #(
      pan_gesture_activation_mode.Move,
      pan_gesture.from_config(
        pan_gesture.Config(
          ..pan_gesture.default_config(),
          activation_mode: pan_gesture_activation_mode.Move,
        ),
      ),
    ),
  ]

  list.each(cases, fn(c) {
    let #(field, expected) = c

    pan_gesture.activation_mode(mod, field)
    |> should.equal(expected)
  })
}

pub fn pan_gesture_min_displacement_test() {
  let mod = pan_gesture.new()
  let cases = [
    #(
      42.0,
      pan_gesture.from_config(
        pan_gesture.Config(
          ..pan_gesture.default_config(),
          min_displacement: 42.0,
        ),
      ),
    ),
  ]

  list.each(cases, fn(c) {
    let #(field, expected) = c

    pan_gesture.min_displacement(mod, field)
    |> should.equal(expected)
  })
}

pub fn pan_gesture_lock_axis_test() {
  let mod = pan_gesture.new()
  let cases = [
    #(
      pan_gesture_lock_axis.X,
      pan_gesture.from_config(
        pan_gesture.Config(
          ..pan_gesture.default_config(),
          lock_axis: pan_gesture_lock_axis.X,
        ),
      ),
    ),
  ]

  list.each(cases, fn(c) {
    let #(field, expected) = c

    pan_gesture.lock_axis(mod, field)
    |> should.equal(expected)
  })
}

pub fn pan_gesture_axis_threshold_test() {
  let mod = pan_gesture.new()
  let cases = [
    #(
      42.0,
      pan_gesture.from_config(
        pan_gesture.Config(..pan_gesture.default_config(), axis_threshold: 42.0),
      ),
    ),
  ]

  list.each(cases, fn(c) {
    let #(field, expected) = c

    pan_gesture.axis_threshold(mod, field)
    |> should.equal(expected)
  })
}

pub fn pan_gesture_delta_threshold_test() {
  let mod = pan_gesture.new()
  let cases = [
    #(
      42.0,
      pan_gesture.from_config(
        pan_gesture.Config(
          ..pan_gesture.default_config(),
          delta_threshold: 42.0,
        ),
      ),
    ),
  ]

  list.each(cases, fn(c) {
    let #(field, expected) = c

    pan_gesture.delta_threshold(mod, field)
    |> should.equal(expected)
  })
}

pub fn pan_gesture_max_press_interval_test() {
  let mod = pan_gesture.new()
  let cases = [
    #(
      42.0,
      pan_gesture.from_config(
        pan_gesture.Config(
          ..pan_gesture.default_config(),
          max_press_interval: 42.0,
        ),
      ),
    ),
  ]

  list.each(cases, fn(c) {
    let #(field, expected) = c

    pan_gesture.max_press_interval(mod, field)
    |> should.equal(expected)
  })
}

pub fn pan_gesture_render_test() {
  let mod = pan_gesture.new()

  let mod_for = pan_gesture.new() |> pan_gesture.for(Some("test"))
  let mod_buttons =
    pan_gesture.new() |> pan_gesture.buttons([gesture_input_button.Secondary])
  let mod_pointer_types =
    pan_gesture.new() |> pan_gesture.pointer_types([pointer_type.Mouse])
  let mod_disabled =
    pan_gesture.new() |> pan_gesture.disabled(pan_gesture.IsDisabled)
  let mod_priority = pan_gesture.new() |> pan_gesture.priority("test")
  let mod_pointers = pan_gesture.new() |> pan_gesture.pointers(42.0)
  let mod_activation_mode =
    pan_gesture.new()
    |> pan_gesture.activation_mode(pan_gesture_activation_mode.Move)
  let mod_min_displacement =
    pan_gesture.new() |> pan_gesture.min_displacement(42.0)
  let mod_lock_axis =
    pan_gesture.new() |> pan_gesture.lock_axis(pan_gesture_lock_axis.X)
  let mod_axis_threshold = pan_gesture.new() |> pan_gesture.axis_threshold(42.0)
  let mod_delta_threshold =
    pan_gesture.new() |> pan_gesture.delta_threshold(42.0)
  let mod_max_press_interval =
    pan_gesture.new() |> pan_gesture.max_press_interval(42.0)
  let cases = [
    #(#(mod, []), element.element("m3e-pan-gesture", [], [])),
    #(
      #(mod, [attribute.id("id")]),
      element.element("m3e-pan-gesture", [attribute.id("id")], []),
    ),

    #(
      #(mod_for, []),
      element.element(
        "m3e-pan-gesture",
        [attribute.attribute("for", "test")],
        [],
      ),
    ),
    #(
      #(mod_buttons, []),
      element.element(
        "m3e-pan-gesture",
        [
          attribute.attribute(
            "buttons",
            gesture_input_button.to_string(gesture_input_button.Secondary),
          ),
        ],
        [],
      ),
    ),
    #(
      #(mod_pointer_types, []),
      element.element(
        "m3e-pan-gesture",
        [
          attribute.attribute(
            "pointer-types",
            pointer_type.to_string(pointer_type.Mouse),
          ),
        ],
        [],
      ),
    ),
    #(
      #(mod_disabled, []),
      element.element(
        "m3e-pan-gesture",
        [attribute.attribute("disabled", "")],
        [],
      ),
    ),
    #(
      #(mod_priority, []),
      element.element(
        "m3e-pan-gesture",
        [attribute.attribute("priority", "test")],
        [],
      ),
    ),
    #(
      #(mod_pointers, []),
      element.element(
        "m3e-pan-gesture",
        [attribute.attribute("pointers", "42.0")],
        [],
      ),
    ),
    #(
      #(mod_activation_mode, []),
      element.element(
        "m3e-pan-gesture",
        [
          attribute.attribute(
            "activation-mode",
            pan_gesture_activation_mode.to_string(
              pan_gesture_activation_mode.Move,
            ),
          ),
        ],
        [],
      ),
    ),
    #(
      #(mod_min_displacement, []),
      element.element(
        "m3e-pan-gesture",
        [attribute.attribute("min-displacement", "42.0")],
        [],
      ),
    ),
    #(
      #(mod_lock_axis, []),
      element.element(
        "m3e-pan-gesture",
        [
          attribute.attribute(
            "lock-axis",
            pan_gesture_lock_axis.to_string(pan_gesture_lock_axis.X),
          ),
        ],
        [],
      ),
    ),
    #(
      #(mod_axis_threshold, []),
      element.element(
        "m3e-pan-gesture",
        [attribute.attribute("axis-threshold", "42.0")],
        [],
      ),
    ),
    #(
      #(mod_delta_threshold, []),
      element.element(
        "m3e-pan-gesture",
        [attribute.attribute("delta-threshold", "42.0")],
        [],
      ),
    ),
    #(
      #(mod_max_press_interval, []),
      element.element(
        "m3e-pan-gesture",
        [attribute.attribute("max-press-interval", "42.0")],
        [],
      ),
    ),
  ]

  list.each(cases, fn(c) {
    let #(#(mod, attributes), expected) = c

    pan_gesture.render(mod, attributes)
    |> should.equal(expected)
  })
}
